...
export default function Login({ isCreateUser }) {
  const {
    handleSubmit,
    register,
    formState: { errors },
  ...
  return (
    <main className="mt-10 mb-12 max-w-[544px] p-4 sm:p-[0px] flex justify-center  flex-col items-center mx-auto pt-9">
      ...
      <p className="text-[16px] text-center mt-3">
        {isCreateUser ? "Already have an account?" : " New to DEV Community?"}
        <Link
          href={!isCreateUser ? "/new-user" : "/login"}
          className="text-[#3b49df] pl-1"
        >
          {isCreateUser ? "Log in." : " Create account."}
        </Link>
      </p>
    </main>
  );
